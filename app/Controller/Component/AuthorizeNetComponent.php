...
<?php 
class AuthorizeNetComponent extends Component {
>>>>>>> 7fda71fbc79600e3186b4711bd43ee725e6d52ed
...
<<<<<<< HEAD
    ...
    public function chargeCard($url, $loginid, $trankey, $ccnum, $ccexpmonth, $ccexpyear, $ccver, $live, $amount, $tax, $shipping, $desc, $billinginfo, $email, $phone, $shippinginfo)
    {
...
<<<<<<< HEAD
            ...
            if ($responsearray[1] == 1) {
                ...
                $output['response']['transaction_id'] = $responsearray[7];
                $output['response']['md5_hash'] = $responsearray[38];
                $output['response']['amount'] = $responsearray[10];

            } else {
                $output['response']['code'] = $responsearray[1];
                $output['response']['reason_code'] = $responsearray[2];
                $output['response']['message'] = $responsearray[4];
            }
            //$responsearray[1] = Response Code (1 = Approved, 2 = Declined, 3 = Error, 4 = Held for Review)
            //$responsearray[2] = Response Subcode (Code used for Internal Transaction Details)
            //$responsearray[3] = Response Reason Code (Code detailing response code)
            //$responsearray[4] = Response Reason Text (Text detailing response code and response reason code)
            //$responsearray[5] = Authorization Code (Authorization or approval code - 6 characters)
            //$responsearray[6] = AVS Response (Address Verification Service response code - A, B, E, G, N, P, R, S, U, W, X, Y, Z)
            //                (A, P, W, X, Y, Z are default AVS confirmation settings - Use your Authorize.net Merchant Interface to change these settings)
            //                (B, E, G, N, R, S, U are default AVS rejection settings - Use your Authorize.net Merchant Interface to change these settings)
            //$responsearray[7] = Transaction ID (Gateway assigned id number for the transaction)
            //$responsearray[38] = MD5 Hash (Gateway generated MD5 has used to authenticate transaction response)
            //$responsearray[39] = Card Code Response (CCV Card Code Verification response code - M = Match, N = No Match, P = No Processed, S = Should have been present, U = Issuer unable to process request)
        }
